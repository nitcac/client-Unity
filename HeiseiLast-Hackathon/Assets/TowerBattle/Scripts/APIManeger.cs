...
public class APIManeger : MonoBehaviour
{
    ...
    {
        uiManeger.SetRankingText("rankingTextをいれてけ～～～");
    }

    public void Request()
    {
        uiManeger.SetRankingText("Roading...");
        StartCoroutine(Post());
    }
    IEnumerator Post()
    {
        UnityWebRequest request = UnityWebRequest.Post("http://ik1-309-14793.vs.sakura.ne.jp/api/htb/ranking", "");
        string postText = "{\"player_name\": %s, \"" + scoreManeger.PutScore + "\": %d}";
        Debug.Log(scoreManeger.PutScore);
        byte[] bodyRaw = Encoding.UTF8.GetBytes(postText);
        request.uploadHandler = (UploadHandler)new UploadHandlerRaw(bodyRaw);
        byte[] rankingJson = request.downloadHandler.data;
        Debug.Log(rankingJson);
        yield break;
    }
